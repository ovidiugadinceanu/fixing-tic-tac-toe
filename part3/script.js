...
function getWinner() {
  return A1 === PLAYER_WITH_X && A2 === PLAYER_WITH_X && A3 === PLAYER_WITH_X
    ? PLAYER_WITH_X
...
function displayWinner(whoWon) {
  ...
  // PLAYER_WITH_O
  // TIE_BETWEEN_X_AND_O
  // see if whoWon matches any of the above variables and display via an alert an appropriate message
  if (whoWon === PLAYER_WITH_X) {
    alert("Player with X has won");
  }
  if (whoWon === PLAYER_WITH_O) {
    alert("Player with Y has won");
  }
  if (whoWon === TIE_BETWEEN_X_AND_O) {
    alert("It's a tie !");
  }
}
